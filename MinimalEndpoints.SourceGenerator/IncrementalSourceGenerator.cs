...
public sealed class IncrementalSourceGenerator : IIncrementalGenerator
{
    ...
    private const string ConfigureUsingAttribute = "Nocpad.AspNetCore.MinimalEndpoints.ConfigureUsingAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterImplementationSourceOutput(compilation.Combine(endpoints.Collect()), static (spc, source) => Execute(source, spc, "IndividualEndpoints", "Individual"));
        context.RegisterImplementationSourceOutput(compilation.Combine(endpointsWithGroup.Collect()), static (spc, source) => Execute(source, spc, "EndpointGroups", "Groups"));
    ...
    internal static void AddStaticSources(IncrementalGeneratorPostInitializationContext context)
    {
        ...
        sb.Append($$"""
            ...
            public static partial class MinimalEndpointsServiceCollectionExtensions
            {
                public static IServiceCollection AddMinimalEndpoints(this IServiceCollection services)
                {
                    return services
                        .AddIndividualEndpoints()
                        .AddEndpointGroups();
                }

                public static partial IServiceCollection AddIndividualEndpoints(this IServiceCollection services);

                public static partial IServiceCollection AddEndpointGroups(this IServiceCollection services);
            }


            [AttributeUsage(AttributeTargets.Method, AllowMultiple = false, Inherited = false)]
            public sealed class ConfigureUsingAttribute : Attribute
            {
                public ConfigureUsingAttribute(string methodName) => MethodName = methodName;
    ...
    private static void Execute((Compilation Left, ImmutableArray<List<Endpoint>> Right) source, SourceProductionContext spc, string implementingMethod, string hintName)
    {
        ...
        sb.Append($$"""
            public static partial class MinimalEndpointsServiceCollectionExtensions
            {
                public static partial IServiceCollection Add{{implementingMethod}}(this IServiceCollection services)
                {                   
            """);
    ...
    private static void AddEndpointsWithoutGroup(List<Endpoint> endpoints, string parent, string variablePrefix, StringBuilder sb)
    {
        ...
        foreach (var item in endpoints)
        {
            ...
            sb.AppendLine();
            item.AppendEndpointString(sb, parent, endpointVariableName);
